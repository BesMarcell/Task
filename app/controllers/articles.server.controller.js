...
exports.update = function(req, res) {
    ...
    article.linkFollows = req.body.linkFollows;
...
exports.findByTag = function(req, res, next, tag) {
    // Use the model
    Article.find({tags: tag}).exec(function(err, tagSel) {
        if (err) return next(err);
        if (!tagSel) return next(new Error('Failed to load tag ' + tagSel));

        // If an tag is found use the 'request' object to pass it to the next middleware
        req.tagSel = tagSel;
        // Call the next middleware
        next();
    });
...
exports.findByWebsiteShort = function(req, res, next, websiteShort) {
    // Use the model
    Article.find({websiteShort: websiteShort}).exec(function(err, article) {
        if (err) return next(err);
        if (!article) return next(new Error('Failed to load website ' + article));
...
exports.readWebsiteShort = function(req, res) {
    res.json(req.article);
    //    res.redirect(payment.getPaymentUrl( req.article[0].website));
//    res.redirect(200,req.article[0].website);
